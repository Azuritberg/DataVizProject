...
document.querySelector(".btn--ethnicity").addEventListener("click", () => {
  currentType = currentType === "ethnicity" ? null : "ethnicity";
  updateChart();
});

document.querySelector(".btn-over-time").addEventListener("click", () => {
  currentMode = currentMode === "time" ? null : "time";
  updateChart();
});

document.querySelector(".btn-over-all").addEventListener("click", () => {
  currentMode = currentMode === "average" ? null : "average";
  updateChart();
});
...
const typeClass = currentType === "gender" ? "gender-active" : "ethnicity-active";

document.querySelectorAll(`.btn--${currentType}`).forEach(btn => {
...
if (!currentMode  || !currentMode) {
  currentMode = "average"; // går även att vänta tills användaren klickar på en knapp
}


  // === Lägg till aktiv stil för Over Time/All baserat på typ + mode ===
const modeBtnClass = currentMode === "average" ? ".btn-over-all" : ".btn-over-time";
const activeClass = currentType === "gender" ? "gender-active" : "ethnicity-active";
